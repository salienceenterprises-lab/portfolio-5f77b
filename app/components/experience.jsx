...
export default function PortfolioExperience({ data }) {
  ...
  return (
    <section id="experience" className="py-28 px-6 bg-[linear-gradient(180deg,#000000_0%,#3D0000_100%)]">
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-xs font-semibold tracking-[0.25em] uppercase text-[#FF0000]">Experience</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4 tracking-tight">Work History</h2>
          <p className="text-white/40 max-w-lg mb-14">Where I've worked and what I've built.</p>
        </motion.div>
                ...
                <motion.div whileHover={{ x: 6 }} transition={{ type: "spring", stiffness: 300 }} className="group relative">
                  ...
                  <div className="relative bg-white/[0.03] border border-white/[0.06] rounded-2xl p-7 transition-all duration-500 group-hover:border-[#FF0000]/30 backdrop-blur-sm">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-4">
                      <div>
                        <h3 className="text-lg font-semibold text-white group-hover:text-red-300 transition-colors">{job.role}</h3>
                        <div className="flex items-center gap-2 mt-1">
                          <FaBriefcase className="w-3.5 h-3.5 text-red-400/70" />
                          <span className="text-sm text-red-400/85 font-medium">{job.company}</span>
                        </div>
                      </div>
                      <div className="flex flex-col items-start sm:items-end gap-1 flex-shrink-0">
                        <span className="text-xs font-medium text-white/30 bg-white/[0.04] border border-white/[0.06] px-3 py-1 rounded-full">{job.period}</span>
                        {job.location && <span className="text-xs text-white/25">{job.location}</span>}
                      </div>
                    </div>
                    ...
                    {job.stack && Array.isArray(job.stack) && (
                      <div className="flex flex-wrap gap-2 pt-4 border-t border-white/[0.05]">
                        {job.stack.map((tech) => (
                          <span key={tech} className="text-[11px] font-medium text-red-300/75 bg-red-900/20 px-2.5 py-1 rounded-md border border-red-700/15">{tech}</span>
                        ))}
                      </div>
                    )}
                  </div>
